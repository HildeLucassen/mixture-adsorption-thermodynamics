...
These folders are **self-contained runs**: each has its own `config.in`, `Input/`, `run.py`, and (after a run) `Output/`. The table below goes **01 → 07** from simpler to more options (data mode, HOA paths, mixture vs pure, storage density, per-component SD, …); you can still open and run **any** folder on its own.
...
|---|--------|-----------------------------|
| **01** | `01_pure_fitting_simple` | **Pure** (e.g. R125 on Bhathia). **`DATA_SOURCE=fitting`** with **`DATA_FILE_FITTING`** only — no RASPA points file required for the main path. **`HEAT_OF_ADSORPTION=cc`**; no Virial block, no storage density. Good baseline before turning on points, Virial, or SD. |
